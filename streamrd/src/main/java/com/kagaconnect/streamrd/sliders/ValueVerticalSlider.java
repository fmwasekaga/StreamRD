...
public class ValueVerticalSlider extends ScrollView {
    ...
    public ValueVerticalSlider(final Context context, String[] values) {
        super(context);
        this.values = values;
    ...
    public ValueVerticalSlider(final Context context) {
        super(context);
        this.context = context;
        setVerticalScrollBarEnabled(false);
    }
    ...
    protected void onScrollChanged(int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
        final float scrollYPosition = getScrollPosition(scrollY);
        String newValue = getSelectedValue(scrollYPosition);
        if (!Objects.equals(newValue, currentValue)) {
            onValueChange(newValue, currentValue);
            currentValue = newValue;
        }
        onScroll(scrollYPosition);

        Timer t = new Timer();
        t.schedule(new TimerTask() {
            @Override
            public synchronized void run() {
                float newScrollYPosition = getScrollPosition(getScrollY());
                if (scrollYPosition == newScrollYPosition && newScrollYPosition != scrollEndedAt) {
                    scrollEndedAt = newScrollYPosition;
                    onScrollEnd(newScrollYPosition);
                }
            }
        }, 100);
    }
