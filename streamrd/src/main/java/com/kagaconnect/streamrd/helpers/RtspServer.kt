            ...
            while (!interrupted()) {
                try {
                    ...
                    if (action.contains("play", true)) {
                        ...
                        commandsManager.sps?.let { commandsManager.pps?.let { it1 -> rtspSender.setVideoInfo(it, it1, commandsManager.vps) } }
                        rtspSender.setAudioInfo(sampleRate)
                        commandsManager.clientIp?.let { rtspSender.setDataStream(socket.getOutputStream(), it) }
